    ...
    if selected:
        async with state.proxy() as data:
            try:
                ...
                await callback_query.message.reply(f'Вы выбрали {date.date()} для отъезда.')
            except ValueError:
                await callback_query.message.reply('Введено неверное значение!\nВыберите дату отъезда еще раз.',
                                                   reply_markup=await SimpleCalendar().start_calendar()
                                                   )
                return
        await FSMLowPrice.next()
...
def register_lowprice_handler(dp: Dispatcher):
    dp.register_message_handler(start, commands=['Низкая_цена'], state=None)
    ...
    dp.register_message_handler(get_city,state=FSMLowPrice.city)
    dp.register_callback_query_handler(callback_id, cb.filter(), state=FSMLowPrice.dest_id)
    dp.register_callback_query_handler(people, ten_cb.filter(), state=FSMLowPrice.people_amount)
    dp.register_callback_query_handler(date_start, simple_cal_callback.filter(), state=FSMLowPrice.date_start)
    dp.register_callback_query_handler(date_end, simple_cal_callback.filter(), state=FSMLowPrice.date_end)
    dp.register_callback_query_handler(get_hotels_num, twenty_five_cb.filter(), state=FSMLowPrice.hotels)
    dp.register_callback_query_handler(stop_show_photos, yes_no_cb.filter(), state=FSMLowPrice.photos)
    dp.register_callback_query_handler(photos_count, ten_cb.filter(), state=FSMLowPrice.photos_count)
